...
//  This constructor generates a matrix of dimensions rows by cols, with all
//  elements initialized to zero The assignment of rows to rows_ using the
//  initialization syntax "rows_(rows)" serves as a shortcut, eliminating the
//  need for an additional assignment statement "rows_ = rows"
...
void S21Matrix::set_cols(int cols) {
  ...
  if (cols != cols_) {
    for (int i = 0; i < rows_; i++) {
      ...
      for (int j = 0; j < cols; ++j) {
        if (j < cols_) pi_temp[j] = matrix_[i][j];
      }
      delete[] matrix_[i];
      matrix_[i] = pi_temp;
    }
  ...
  //    if (cols != cols_) {
  //        for (int i = 0; i < rows_; i++) {
  //            // Allocate a new array of double with 'cols' elements,
  //            initialized to 0.0 double* pi_temp = new double[cols]();
...
bool S21Matrix::EqMatrix(const S21Matrix& other) {
  ...
  //    res = memcmp(matrix_[0], other.matrix_[0], sizeof(matrix_[0]) * rows_ *
  //    cols_) == 0;
  return res;
}
...
// This function multiplies the current matrix object with the other matrix
// object passed as an argument.
void S21Matrix::MulMatrix(const S21Matrix& other) {
  // Check if the number of columns in the current matrix is equal to the number
  // of rows in the other matrix
  if (cols_ != other.rows_) {
    throw std::out_of_range(
        "Multiply matrix only when first matrix cols == second matrix rows");
  }
  // Create a copy of the current matrix object
  S21Matrix copy(*this);
  // Set the number of columns in the current matrix object to be equal to the
  // number of columns in the other matrix
  ...
  for (int i = 0; i < rows_; i++) {
    for (int j = 0; j < cols_; j++) {
      ...
      for (int k = 0; k < copy.cols_; ++k) {
        // Multiply element at ith row and kth column of first matrix with
        // element at kth row and jth column of second matrix
...
// This function returns a new matrix object that is the transpose of the
// current matrix object.
S21Matrix S21Matrix::Transpose() {
  // First, we create a new matrix object with the number of rows and columns
  // swapped. This is because the transpose of a matrix will have the number of
  // rows and columns flipped.
  S21Matrix res(cols_, rows_);
  // Next, we iterate over the rows and columns of the current matrix and set
  // the elements of the resulting matrix to be the transpose of the current
  // matrix.
  for (int i = 0; i < rows_; i++)
    for (int j = 0; j < cols_; j++)
      // We set the element at the jth row and ith column of the resulting
      // matrix to be the element at the ith row and jth column of the current
      // matrix.
...
double S21Matrix::GetMinor(int i, int j) {
  ...
  if (rows_ == 2) {
    ...
    // If the matrix is larger than a 2x2 matrix, we need to calculate the minor
    // using the determinant.
  } else {
    int x = 0;
    int y = 0;
    // First, we create a new matrix object that is the matrix formed by
    // deleting the row and column containing the element.
    ...
    // If the resulting matrix is larger than a 2x2 matrix, we calculate the
...
// as an argument and stores the result in the same object. The matrix of
...
// This function returns a new matrix object that is the matrix of complements
...
S21Matrix S21Matrix::CalcComplements() {
  ...
  // Next, we create a copy of the current matrix object and assign it to a new
  // matrix object called result.
...
// This function returns a new matrix object that is the inverse of the current
// matrix object.
S21Matrix S21Matrix::InverseMatrix() {
  // First, we create a new matrix object with the same dimensions as the
  // current matrix object.
  ...
  if (fabs(det) < 1e-6) {
    ...
    if (rows_ == 1) {
      ...
      // If the matrix is larger than a 1x1 matrix, we need to use the matrix of
      // complements to calculate the inverse.
    } else {
      ...
      // Next, we transpose the matrix of complements to get the cofactor
      // matrix.
      result = complement.Transpose();
      // Finally, we multiply the cofactor matrix by 1/determinant to get the
      // inverse matrix.
...
S21Matrix& S21Matrix::operator=(const S21Matrix& other) {
  ...
  if (this == &other)
    // If the current matrix is the same as the other matrix, return the current
    // matrix
    return *this;
  // Set the number of columns of the current matrix to the number of columns of
  // the other matrix
  this->set_cols(other.cols_);
  // Set the number of rows of the current matrix to the number of rows of the
  // other matrix
...
// The first operator overload is the equality operator (==). It checks if the
// current matrix object is equal to another matrix object. It returns a bool
// value indicating whether the two matrices are equal.
bool S21Matrix::operator==(const S21Matrix& other) { return EqMatrix(other); }

// The next operator overloads are the compound assignment operators for
// addition, subtraction, and multiplication. These operator overloads modify
// the current matrix object and return a reference to it.
S21Matrix& S21Matrix::operator+=(const S21Matrix& other) {
  SumMatrix(other);
  return *this;
}
...
// The next operator overloads are the binary operators for addition,
// subtraction, and multiplication. These operator overloads create and return a
// new matrix object that is the result of the operation.
...
// The next operator overloads are the compound assignment and binary operators
// for scalar multiplication. These operator overloads multiply each element in
// the matrix by a scalar value.
